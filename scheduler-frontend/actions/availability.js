...
export async function getUserAvailability(firebaseUid) {
  if (!firebaseUid) {
...
export async function getUserAvailabilityByUsername(username) {
  if (!username) {
...
export async function updateAvailability(data, firebaseUid) {
  if (!firebaseUid) {
    throw new Error("Unauthorized");
  }
...
export async function getEventAvailability(eventId) {
  ...
  const bookings = event.bookings; // Only bookings for THIS event
